...
public class PikeperchhillBiome {
	...
	public static Biome createBiome() {
		BiomeSpecialEffects effects = new BiomeSpecialEffects.Builder().fogColor(-1010540).waterColor(-12953494).waterFogColor(-15062477)
				.skyColor(-1010540).foliageColorOverride(-9109426).grassColorOverride(-7602176).build();
		...
		biomeGenerationSettings.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, register("trees", Feature.TREE
				.configured((new TreeConfiguration.TreeConfigurationBuilder(
						new SimpleStateProvider(OrderofobsidianModBlocks.BURNEDTREE_LOG.defaultBlockState()), new ForkingTrunkPlacer(5, 2, 2),
						new SimpleStateProvider(OrderofobsidianModBlocks.BURNEDTREE_LEAVES.defaultBlockState()),
	...
	public static void init() {
		Registry.register(BuiltinRegistries.CONFIGURED_SURFACE_BUILDER, new ResourceLocation(OrderofobsidianMod.MODID, "pikeperchhill"),
				SURFACE_BUILDER);
		CONFIGURED_FEATURES.forEach((resourceLocation, configuredFeature) -> Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, resourceLocation,
				configuredFeature));
		BiomeManager.addBiome(BiomeManager.BiomeType.DESERT, new BiomeManager.BiomeEntry(
				ResourceKey.create(Registry.BIOME_REGISTRY, BuiltinRegistries.BIOME.getKey(OrderofobsidianModBiomes.PIKEPERCHHILL)), 10));
	}

	private static final Map<ResourceLocation, ConfiguredFeature<?, ?>> CONFIGURED_FEATURES = new HashMap<>();

	private static ConfiguredFeature<?, ?> register(String name, ConfiguredFeature<?, ?> configuredFeature) {
		CONFIGURED_FEATURES.put(new ResourceLocation(OrderofobsidianMod.MODID, name + "_pikeperchhill"), configuredFeature);
